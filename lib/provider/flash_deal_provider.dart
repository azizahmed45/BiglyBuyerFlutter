...
import 'package:bigly24/data/model/response/flash_deal_model.dart';
...
class FlashDealProvider extends ChangeNotifier {
  ...
  Future<void> getMegaDealList(bool reload, BuildContext context, String languageCode) async {
    if (_flashDealList.length == 0 || reload) {
      ApiResponse apiResponse = await megaDealRepo.getFlashDeal(languageCode);
      if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
        ...
        if(_flashDeal.id != null) {
          ...
          ApiResponse megaDealResponse = await megaDealRepo.getFlashDealList(_flashDeal.id.toString(), languageCode);
